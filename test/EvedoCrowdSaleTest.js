...
const EvedoCrowdSaleContract = artifacts.require('EvedoCrowdSale')
...
contract('EvedoCrowdSale', function (accounts) {
  let tokenContract
  ...
  let init = async () => {
    ...
    crowdsaleContract = await EvedoCrowdSaleContract.new(2600, creatorAccount, tokenContract.address)
  ...
  describe('Creation', () => {
    ...
    it('should set all stages', async () => {
      ...
      expect(rate).bignumber.to.equal(2600)
      ...
      expect(stage1[0]).bignumber.to.equal(2600)
      expect(web3.fromWei(stage1[1])).bignumber.to.equal(6000)
      expect(stage1[2]).bignumber.to.equal(0)

      let finalStage = await crowdsaleContract.stages.call(6)
      expect(finalStage[0]).bignumber.to.equal(2000)
      expect(web3.fromWei(finalStage[1])).bignumber.to.equal(80000)
      expect(finalStage[2]).bignumber.to.equal(0)
    })
  })

  describe('Stage 1 Sale', () => {
    beforeEach(init)
  ...
  describe('General sale', () => {
    beforeEach(init)

    it('Sender should be able to buy tokens', async () => {
      let initialOwnerEthBalance = await web3.eth.getBalance(creatorAccount)
      crowdsaleContract.setStage(6)
      // when user sends 1 eth to EvedoTokenSale contract
      await crowdsaleContract.sendTransaction({from: userAccount, value: web3.toWei(1, 'ether')})
      let userTokenBalance = await tokenContract.balanceOf.call(userAccount)
      const expectedTokenBalance = new BigNumber(2000).times(new BigNumber(10).pow(decimals))
      expect(userTokenBalance).to.bignumber.equal(expectedTokenBalance)
